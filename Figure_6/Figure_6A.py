...
y_key = 'TGTRP_chi2'                                #feature used here to plot in y-direction

x_data =  txx[:,dica[x_key]]*10                     #assign x_data as 1-D array with x-direction feature (change nm to angtrom)
y_data =  txx[:,dica[y_key]]*180/np.pi              #assign y_data as 1-D array with y-direction feature (change radian to degree)
...
axs.set_xlim([9,33])                                                                            #min and max limit of x-axis
axs.set_ylim([-180,180])                                                                        #min and max limit of y-axis

axs.set_xticks(range(int(9),int(33)+1,4))                                                       #x-axis ticks
axs.set_yticks(range(int(-180),int(180)+1,60))                                                      #y-axis ticks

plt.xticks(fontsize=22)                                                                         #x-axis tick size
plt.yticks(fontsize=22)                                                                         #y-axis tick size

axs.set_xticklabels(range(int(9),int(33)+1,4))                                                  #x-axis ticklabel
axs.set_yticklabels(range(int(-180),int(180)+1,60))                                                 #y-axis ticklabels

plt.xlabel('THC Binding' + ' (\AA)', **hfont,fontsize=30)                                       #x-axis label
plt.ylabel('Toggle Switch ' + 'TRP356$^{6.48}$ Angle' + '($\chi_2$)', **hfont,fontsize=30)                                   #y-axis label
